<?php
// /payment/process-retention.php
// SenangPay server-to-server callback handler for subscription retention payments.
// - Verifies transaction via SenangPay API
// - Updates Subscriptions table with new expiry date
// - Creates Subscription_Billing_History entry with billing_type='retention'
// - Sends retention confirmation email
...
if (defined('DEMO_MODE') && DEMO_MODE) {
    ...
    echo json_encode(['ok' => true, 'note' => '[DEMO] Retention callback stub — no real SenangPay verification.']);
    exit;
}
...
function httpGetJson(string $url, int $timeout = 12): array
{
  ...
  // PHP 8.5-safe cleanup. Avoid curl_close() deprecation warning.
  @curl_reset($ch);
  $ch = null;
...
// ============================================================
// RETENTION BUSINESS UPDATE SECTION
// ============================================================

function markSubscriptionRetention(
  mysqli $conn,
  array $subscriptionRow,
  string $orderId,
  string $txnRef,
  string $retentionAmount,
  bool $isRetentionInstallment = false,
  ?float $retentionInstallmentRemaining = null
): bool {
  $LOG = __DIR__ . '/sp_callback.log';

  if (!tableExists($conn, 'Subscriptions') || !tableExists($conn, 'Subscription_Billing_History')) {
    log_line($LOG, "RETENTION: Subscriptions or Subscription_Billing_History table missing");
    return false;
  }

  $subId = (int)($subscriptionRow['id'] ?? 0);
  if ($subId <= 0) {
    log_line($LOG, "RETENTION: Invalid subscription_id");
    return false;
  }
  ...
  if ($durationValue <= 0 || $currentExpiry === '') {
    log_line($LOG, "RETENTION: Missing duration or expiry_date. duration_value={$durationValue}, expiry_date={$currentExpiry}");
    return false;
  }
  ...
  try {
    ...
    if ($expiryTs === false) {
      log_line($LOG, "RETENTION: Invalid expiry_date format: {$currentExpiry}");
      return false;
    }
  } catch (Exception $e) {
    log_line($LOG, "RETENTION: Exception parsing expiry_date: " . $e->getMessage());
    return false;
  }
  ...
  if ($newExpiryTs === false) {
    log_line($LOG, "RETENTION: Failed to calculate new expiry with interval: {$intervalStr}");
    return false;
  }
  ...
  if ($isRetentionInstallment) {
    // Retention installment: expiry is extended now, but next payment is due next month.
    $nextRenewalDate = date('Y-m-d H:i:s', strtotime('+1 month', strtotime($now)));
  } else {
    // Full retention payment: no installment balance, next cycle is at expiry.
    $nextRenewalDate = date('Y-m-d H:i:s', $newExpiryTs);
  }

  $txnEsc = mysqli_real_escape_string($conn, $txnRef);
  $amountEsc = mysqli_real_escape_string($conn, (string)$retentionAmount);
  $orderIdEsc = mysqli_real_escape_string($conn, $orderId);

  $remainingSql = "";

  if ($isRetentionInstallment && $retentionInstallmentRemaining !== null && $retentionInstallmentRemaining > 0) {
    $remainingEsc = mysqli_real_escape_string($conn, number_format($retentionInstallmentRemaining, 2, '.', ''));
    $remainingSql = ", remaining_month_price = '{$remainingEsc}'";
  } elseif (!$isRetentionInstallment) {
    // Full retention payment clears installment balance for this cycle.
    $remainingSql = ", remaining_month_price = NULL";
  }

  // Update the Subscriptions table with new expiry and retention info
  $retentionOk = mysqli_query($conn, "
  ...
  if (!$retentionOk) {
    log_line($LOG, "RETENTION: Failed to update Subscriptions: " . mysqli_error($conn));
    return false;
  }

  // Check for duplicate retention audit record to prevent idempotency issues
  $billingTypeForDup = $isRetentionInstallment ? 'retention_installment' : 'retention';
  $billingTypeForDupEsc = mysqli_real_escape_string($conn, $billingTypeForDup);

  $checkSql = "
    SELECT id FROM `Subscription_Billing_History`
    WHERE subscription_id = {$subId}
    AND billing_type = '{$billingTypeForDupEsc}'
    AND (order_id = '{$orderIdEsc}'
    OR payment_id = (SELECT id FROM Payment WHERE transaction_ref = '{$txnEsc}' LIMIT 1))";
  ...
  if ($dupAudit && mysqli_num_rows($dupAudit) > 0) {
    log_line($LOG, "RETENTION: Duplicate audit record found for subscription_id={$subId}, order_id={$orderId} - skipping audit insert");
    return true;
  }

  $billingType = $isRetentionInstallment ? 'retention_installment' : 'retention';
  $billingTypeEsc = mysqli_real_escape_string($conn, $billingType);

  $auditCols = "subscription_id, billing_type, amount, paid_at, order_id, created_at";
  $auditValues = "{$subId}, '{$billingTypeEsc}', {$amountEsc}, '{$now}', '{$orderIdEsc}', '{$now}'";
  ...
  if (!$insertOk) {
    log_line($LOG, "RETENTION: Failed to insert Subscription_Billing_History: " . mysqli_error($conn));
    return false;
  }

  log_line($LOG, "RETENTION: Updated subscription_id={$subId}, new_expiry={$newExpiryDate}, retention_amount={$retentionAmount}");
  return true;
}

function applyRetentionDiscount(array $orderRow): array
{
  $discountType = trim((string)($orderRow['retention_discount_type_snapshot'] ?? ''));
  $discountValue = (float)($orderRow['retention_discount_value_snapshot'] ?? 0);
...
function sendRetentionEmail(mysqli $conn, array $subscriptionRow, string $orderId, string $txnRef, string $retentionAmount): void
{
  $LOG = __DIR__ . '/sp_callback.log';
  ...
  if ($email === '' || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
    log_line($LOG, "RETENTION_EMAIL: Invalid email address: {$email}");
    return;
  }
  ...
  $subject = 'Thank You for Staying with Us - Your Subscription Extended';
  ...
  $bodyHtml = "
    ...
    <p>Thank you for staying with us! Your subscription has been extended successfully.</p>
    <div style='margin: 25px 0; padding: 20px; background: rgba(255,255,255,.05); border-left: 4px solid #f59e0b; border-radius: 12px; border: 1px solid rgba(255,255,255,.08)'>
      ...
      <p style='margin: 5px 0;'><strong>Special Retention Offer Amount:</strong> RM " . mail_h($retentionAmount) . "</p>
    ...
    <p>Your subscription has been extended and is now active until " . mail_h($expiryFormatted) . ". We're committed to delivering even better value and service to you.</p>
  ";

  $html = buildMailLayout([
    'subject'   => $subject,
    'body_html' => $bodyHtml,
    'badge_text' => 'Subscription Extended'
  ]);

  $ok = sendBrevoEmail($email, $name, $subject, $html);
  log_line($LOG, "RETENTION_EMAIL: sent=" . ($ok ? 'true' : 'false') . " to={$email}");
}
...
function handleRetention(mysqli $conn, string $order_id, string $txn_ref, string $price): void
{
  $LOG = __DIR__ . '/sp_callback.log';

  log_line($LOG, "--- handleRetention START order={$order_id} txn={$txn_ref} price={$price}");
  ...
  $paymentRow = mysqli_query($conn, "
    SELECT id, subscription_id, subscription_mode, price, remaining_month_price, email, name, phone, verified, status
    FROM `Payment`
  ...
  if ($subscriptionIdFromPayment > 0) {
    $subQuery = mysqli_query($conn, "
      SELECT id, subscription_no, customer_name, customer_email, product_name_snapshot, variant_name_snapshot,
             duration_value, duration_unit, expiry_date, remaining_month_price, retention_price, status, renewal_count
      FROM `Subscriptions`
  ...
  if ($subId === 0) {
    ...
    $cols = "id, subscription_no, customer_name, customer_email, product_name_snapshot, variant_name_snapshot, duration_value, duration_unit, expiry_date, remaining_month_price, retention_price, status, renewal_count";
    $subQuery = mysqli_query($conn, "
      SELECT {$cols}
      FROM `Subscriptions`
  ...
  if (tableExists($conn, 'Orders')) {
    $ordersQuery = mysqli_query($conn, "
      SELECT id, retention_discount_type_snapshot, retention_discount_value_snapshot
      FROM `Orders`
    ...
    if ($ordersQuery && mysqli_num_rows($ordersQuery) > 0) {
      ...
      log_line($LOG, "STEP3 found Orders row with potential retention discount");
    }
  }

  // ── STEP 4: Calculate retention amount with discount ───────
  $paymentSubscriptionMode = strtolower(trim((string)($payData['subscription_mode'] ?? 'retention')));
  $isRetentionInstallment = ($paymentSubscriptionMode === 'retention_installment');

  $retentionInstallmentRemaining = null;

  if ($isRetentionInstallment) {
    // For retention installment, the paid amount is the first retention installment amount.
    $retentionAmount = (float)($payData['price'] ?? 0);
    $retentionInstallmentRemaining = isset($payData['remaining_month_price'])
      ? (float)$payData['remaining_month_price']
      : null;

    log_line($LOG, "STEP4 retention installment detected. first_payment={$retentionAmount}, remaining={$retentionInstallmentRemaining}");
  } else {
    $retentionAmount = (float)($subscriptionRow['retention_price'] ?? 0);

    if ($retentionAmount <= 0) {
      log_line($LOG, "STEP4 retention_price not set, falling back to remaining_month_price");
      $retentionAmount = (float)($subscriptionRow['remaining_month_price'] ?? 0);
    }

    if ($orderRow) {
      $discount = applyRetentionDiscount($orderRow);
      if ($discount['discount_type'] !== '' && $discount['discount_value'] > 0) {
        $originalAmount = $retentionAmount;
        $discountType = strtolower(trim((string)$discount['discount_type']));

        if (in_array($discountType, ['percent', 'percentage'], true)) {
          $retentionAmount = $retentionAmount * (1 - $discount['discount_value'] / 100);
        } elseif ($discountType === 'fixed') {
          $retentionAmount = max(0, $retentionAmount - $discount['discount_value']);
        }
        $retentionAmount = number_format($retentionAmount, 2, '.', '');
        log_line($LOG, "STEP4 retention discount applied: {$discount['discount_type']}({$discount['discount_value']}) = {$originalAmount} -> {$retentionAmount}");
      }
    }
  }

  if ($retentionAmount <= 0) {
    log_line($LOG, "STEP4 invalid retention amount={$retentionAmount}");
    respond(400, 'Invalid retention payment amount');
  }

  log_line($LOG, "STEP4 retention_amount={$retentionAmount}, mode={$paymentSubscriptionMode}");
  ...
  log_line($LOG, "STEP5 Payment row created id={$paymentId}");

  // ── STEP 6: RETENTION BUSINESS UPDATE ──────────────────────
  log_line($LOG, "STEP6 RETENTION BUSINESS UPDATE SECTION");

  $retentionOk = markSubscriptionRetention(
    $conn,
    $subscriptionRow,
    $order_id,
    $txn_ref,
    (string)$retentionAmount,
    $isRetentionInstallment,
    $retentionInstallmentRemaining
  );

  if (!$retentionOk) {
    log_line($LOG, "STEP6 markSubscriptionRetention failed");
    respond(500, 'Failed to update subscription');
  }

  log_line($LOG, "STEP6 subscription retention accepted successfully");
  ...
  // ── STEP 8: Send retention confirmation email ──────────────
  log_line($LOG, "STEP8 sending retention confirmation email");
  sendRetentionEmail($conn, $subscriptionRow, $order_id, $txn_ref, (string)$retentionAmount);
...
log_line(__DIR__ . '/sp_callback.log', 'RETENTION HIT ' . json_encode($_REQUEST));
...
handleRetention($conn, $order_id, $txn_ref, $price);
